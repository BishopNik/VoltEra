export default async function handler(req, res) {
  ...
  const configuredSiteUrl = /^https:\/\/[a-z0-9.-]+(?::\d+)?$/i.test(String(process.env.PUBLIC_SITE_URL || '')) ? String(process.env.PUBLIC_SITE_URL).replace(/\/$/, '') : 'https://www.voltares.pp.ua';
  const siteUrl = configuredSiteUrl.replace(/^https:\/\/voltares\.pp\.ua$/i, 'https://www.voltares.pp.ua');
